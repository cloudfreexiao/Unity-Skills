...
| # | Rule | Source anchor |
|---|------|---------------|
| 1 | `YooAssets.Initialize()` must be called exactly once before any `CreatePackage()`; a `[{nameof(YooAssets)}]` driver GameObject is created via `DontDestroyOnLoad` to pump `OperationSystem.Update()`. Do not `Destroy` this driver yourself. | `Runtime/YooAssets.cs:38-64` |
| 2 | Each `EPlayMode` value pairs with **exactly one** `InitializeParameters` subclass. Passing the wrong subclass throws `NotImplementedException` at runtime. | `Runtime/ResourcePackage/ResourcePackage.cs:107-135, 170-181`, `Runtime/InitializeParameters.cs:8-110` |
