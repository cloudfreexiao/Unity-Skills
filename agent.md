...
| 模式 | 默认 | 行为 |
|------|:----:|------|
| Approval | - | AI 调 FullAuto skill 时返回 `MODE_RESTRICTED` + grant token；用户同意后调一次 `POST /permission/grant`，**服务端一步执行原 skill 并在 grant 响应中返回 `executed: true, result`**（AI 不再需要重放 skill 调用）。grant 后单次执行；持久放行需用户在面板手动加入 Allowlist |
...
**双轨审批**（仅 Approval 模式）：
- Dialog：默认；AI 对话说明 + grant token → 用户文字同意 → AI 调一次 `/permission/grant` → 服务端一步执行并返回 `result`
- Panel：面板开启 `Panel Approval Required` 后，AI 必须先告知用户去 Unity 面板点 `[Approve]`，等用户点过再调 `/permission/grant`（同样一步执行返回 `result`）；提前 grant 返回 `GRANT_PENDING_APPROVAL`

**Allowlist（用户白名单，用户管理的永久放行）**：用户在面板 `Allowlist Skills` 区域手动添加；命中后所有调用直接执行（**可覆盖 Delete / PlayMode / Reload 等高危拦截**）；AI 一般不应主动 add/remove。

**切换方式**：仅 `Window > UnitySkills > Server` 面板；不再支持对话触发词。

**REST 端点**：`GET /permission/status`、`POST /permission/{grant,approve,deny}`；`GET /permission/allowlist`、`POST /permission/allowlist/{add,remove}`；`POST /permission/revoke`（deprecated，转发到 `allowlist/remove`）；`GET /permission/audit`；`GET /health` 含 `currentMode` / `panelApprovalRequired` / `pendingCount`；`GET /skills` 每条含 `mode` 字段。`/permission/status` 字段 `granted` 重命名为 `allowlist`（旧字段保留一版本作为兼容别名）。

**老安装零感知**：检测旧版 `UnitySkills_*` EditorPrefs key 自动默认 Bypass，与原 Full-Auto 行为一致。

**编辑器 UI（v1.9）**：
- `Window > UnitySkills > Server`：模式三档单选 / Panel Approval 开关 / 待批列表 / 白名单列表（Allowlist Skills / `+ Add Skill` / `[Remove]` / `[Clear All]`） / `[View Audit Log]`。
